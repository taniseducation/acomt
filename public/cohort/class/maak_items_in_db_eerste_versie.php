...
foreach ($loadedSheetNames as $vakCode) {
    ...
    for ($rij=2;$rij<=43;$rij++) {        
        if (($rij-2) % 7 == 0) {
            ...
            $sql = "select * from cohorten where vid = {$sheetData[$rij]['B']} and beginJaar={$beginJaar[$sheetData[$rij]['A']]} and niveau='{$niveau[1]}'";
            $record = mysqli_query($DBverbinding, $sql);
            if (mysqli_num_rows($record) == 1) {
                $cohort = mysqli_fetch_assoc($record);
            }
            else {
                echo '<h3>'.$sql.'<br>Fatale fout.</h3>';
                die();
            }
            ...
            for ($n=$rij; $n<$rij+6;$n++) {
                ...
                */
                ...
                if ($sheetData[$n]['H'] != '0' && $sheetData[$n]['A'] == '4A' && $sheetData[$n]['D'] == 'IF') {
                    ...
                    $inTW = false;
                    if ($sheetData[$n]['J'] == 'tt' && $sheetData[$n]['M']) {$inTW = true;}
                    ...
                    echo "<h4>$sql</h4>";
                    // uitgezet voor dubbelingen mysqli_query($DBverbinding, $sql);
                }
                else {
